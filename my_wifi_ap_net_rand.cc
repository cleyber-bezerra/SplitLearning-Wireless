...
int main(int argc, char *argv[])
{
    ...
    for (uint32_t j = 0; j < wifiStaNodes.GetN(); ++j)
    {
        ...
        if (liIonSourcePtr != nullptr) {
            initialEnergies[j] = liIonSourcePtr->GetRemainingEnergy();
        } else {
            NS_LOG_ERROR("LiIonEnergySource is null for node " << j);
                ...
                if (ipv4 != nullptr) {
                    ...
                    if (addr == t.sourceAddress)
                    {
                        ...
                        if (liIonSourcePtr != nullptr) {
                            ...
                            NS_LOG_ERROR("LiIonEnergySource is null for client " << clientId);
                        }

                        break;
                    }
                } else {
                    NS_LOG_ERROR("Ipv4 is null for node " << nodePair.second);
                }
            }
