...
def process_file(file_path, color, label):
    ...
    grouped = df.groupby('Flow ID')['Delay (s)'].agg(['mean', 'std', 'count'])
    ...
    clients_complete = np.arange(1, 4)
...
plt.title('Average delay for each customer')
plt.xlabel('Clients')
plt.ylabel('Delay (s)')
plt.grid(True)
plt.legend(loc='upper right')
plt.xticks(np.arange(1, 4), rotation=0)
