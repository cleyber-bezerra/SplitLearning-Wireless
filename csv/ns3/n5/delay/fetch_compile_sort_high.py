...
columns_to_drop = ['Client', 'Source Address', 'Destination Address', 'Tx Packets', 'Rx Packets', 'Throughput (Mbps)', 'Packet Loss Ratio (%)','Transmission Time (s)','Energy Consumed (J)']
...
combined_df_cleaned['Delay (s)'] = combined_df_cleaned['Delay (s)'].apply(lambda x: float(x.strip('[]')) if isinstance(x, str) else float(x))
...
dtype_map = {
    ...
    'Delay (s)': float,         # Assuming 'rx_packets' is integer
