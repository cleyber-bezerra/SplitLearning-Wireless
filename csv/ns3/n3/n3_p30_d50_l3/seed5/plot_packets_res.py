...
def process_file(file_path, color, label):
    ...
    grouped = df.groupby('Client')['Validation Accuracy'].agg(['mean', 'std', 'count'])
    ...
    clients_complete = np.arange(1, 21)
...
plt.title('Average Accuracy for Each Client')
plt.xlabel('Clients')
plt.ylabel('Validation Accuracy')
plt.grid(True)
plt.legend(loc='upper right')
plt.xticks(np.arange(1, 21), rotation=0)
