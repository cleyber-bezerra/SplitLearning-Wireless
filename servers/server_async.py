...
for user in user_info:
    print(f"Waiting to receive request from client {user['addr']}")
    recvreq = user["conn"].recv(1024)
    print(f"Received request message from client {user['addr']}: {recvreq.decode()}")
    #print("receive request message from client <{}>".format(user["addr"]))
    user["conn"].sendall(DAM)
    print(f"Sent response message to client {user['addr']}")
