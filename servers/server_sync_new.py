...
FLAG = 0  # 0 - todos com latência, 1 - filtro de latências

if FLAG == 1:
    filtered_clients = [client_in[i] for i in range(len(delay_in)) if delay_in[i] <= 0.40 and delay_in[i] != float('inf')]
    filtered_delay = [delay_in[i] for i in range(len(delay_in)) if delay_in[i] <= 0.40 and delay_in[i] != float('inf')]
...
for num_user in range(USER):
    ...
    connection_time = (end_conn_time - start_conn_time) * 1000  # Tempo de conexão em ms

    # Inclui o atraso do cliente no tempo de conexão
    delay = delays[num_user]  # Atraso específico para o cliente
    conn.sendall(f"{connection_time},{delay}".encode())
